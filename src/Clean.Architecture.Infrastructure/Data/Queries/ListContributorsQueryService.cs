...
public class ListContributorsQueryService(AppDbContext _db) : IListContributorsQueryService
{
  ...
  public async Task<IEnumerable<ContributorDTO>> ListAsync()
  {
    ...
    var result = await _db.Contributors.FromSqlRaw("SELECT Id, FirstName, LastName FROM Contributors") // don't fetch other big columns
      .Select(c => new ContributorDTO(c.Id, c.FirstName, c.LastName))
      .ToListAsync();
